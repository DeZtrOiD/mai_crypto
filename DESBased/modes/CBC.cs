...
public class CBCMode : ICipherMode {
    ...
    public byte[] Encrypt(in byte[] plaintext) {
        ...
        for (int i = 0; i < blocks.Length; i++) {
            byte[] xor = ByteUtils.Xor(blocks[i], prev);
            byte[] cipherBlock = _cipher.Encrypt(xor);
            result[i] = cipherBlock;
            prev = cipherBlock;
        }
    ...
    public byte[] Decrypt(in byte[] ciphertext) {
        ...
        Parallel.For(0, blocks.Length, i => {
            decryptedBlocks[i] = _cipher.Decrypt(blocks[i]);
            result[i] = ByteUtils.Xor(
                _cipher.Decrypt(blocks[i]),
                i == 0 ? (byte[])_iv!.Clone() : blocks[i - 1]
            );
        });
    ...
    private static void ValidateInput(IBlockCipher? cipher, byte[]? data, byte[]? _iv) {
        ...
        if (data.Length % cipher.BlockSize != 0)
            throw new ArgumentException($"Data length must be multiple of block size ({ cipher.BlockSize } bytes).");
    }
}
}
